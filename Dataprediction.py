...
def fetch_metrics():
    ...
    print("[INFO] Organized metrics by type:")
    for metric_type, values in metrics_by_type.items():
        print(f"  - {metric_type}: {values[:5]}... ({len(values)} total values)")
    return metrics_by_type

def predict_future_value(data_points):
    """Predicts future values using a moving average with a momentum adjustment."""
    print(f"[INFO] Calculating prediction for data points: {data_points[:5]}... ({len(data_points)} total points)")
    if not data_points:
        print("[WARNING] No data points available to calculate prediction.")
        return None
    moving_avg = statistics.mean(data_points)
    prediction = moving_avg + (moving_avg * 0.1)  # Simple trend adjustment
    print(f"[INFO] Predicted value: {prediction:.2f} (Moving Avg: {moving_avg:.2f})")
    return prediction

def generate_predictions():
    """Generates predictions for all metrics."""
    print(f"\n=== Generating Predictions at {datetime.now().isoformat()} ===")
    metrics_by_type = fetch_metrics()
    predictions = {}
    for metric_type, values in metrics_by_type.items():
        print(f"[INFO] Generating prediction for metric: {metric_type}")
        predictions[metric_type] = predict_future_value(values)
    return predictions
...
def main():
    ...
    for metric, prediction in predictions.items():
        if prediction is not None:
            print(f"[INFO] {metric}: {prediction:.2f}")
        else:
            print(f"[WARNING] {metric}: No data available for prediction.")
    print("[INFO] Prediction process complete.")
