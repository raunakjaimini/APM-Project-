...
def upsert_endpoint_data(cursor, endpoint_name, request_count, timestamp):
    """
    Insert or update the request count and timestamp for an endpoint in the database.
    """
    try:
        ...
        cursor.execute('''
            ...
            VALUES (%s, %s, %s)
            ON CONFLICT (name_endpoints)
            DO UPDATE SET 
                request_counts = EXCLUDED.request_counts,
                last_updated = EXCLUDED.last_updated;
        ''', (endpoint_name, request_count, timestamp))
    except Exception as e:
        print(f"[ERROR] Failed to upsert data for endpoint '{endpoint_name}': {e}")
...
def monitor_request_counts(log_file="request_counts_log.json"):
    ...
    """
    url = "http://localhost:8080/monitor/request_counts"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for endpoint, count in log_data["endpoints"].items():
                upsert_endpoint_data(cursor, endpoint, count, log_data["timestamp"])
